...
showProfileMusic=function(){$.post('system/box/edit_profile_music.php',{token:utk,},function(response){overModal(response,500);});}
saveProfileMusic=function(){var file_data=$("#uploadProfileMusic").prop("files")[0];var music_file=($("#uploadProfileMusic")[0].files[0].size/1024/1024).toFixed(2);if(music_file>fmw){callSaved(system.fileBig,3);}
else if($("#uploadProfileMusic").val()===""){callSaved(system.noFile,3);}
else{if(waitUpload==0){waitUpload=1;uploadIcon('chat_file_icon_new',1);var form_data=new FormData();form_data.append("file",file_data)
form_data.append("token",utk)
$.ajax({url:"system/action_profile_music.php",dataType:'script',cache:false,contentType:false,processData:false,data:form_data,type:'post',success:function(response){if(response==11){callSaved(system.saved,1);}
else if(response==33){callSaved(system.wrongFile,3);}
uploadIcon('chat_file_icon_new',2);waitUpload=0;location.reload();}})}
else{return false;}}}
openPlayer = function(){
	$('#player_box').toggle();
}