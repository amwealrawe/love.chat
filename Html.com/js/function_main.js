$(document).ready(function() {
	let speed = 10; // تهيئة المتغير هنا في بداية الدالة
  
	// بقية الكود الخاص بك
	$('#example').animate({ left: '100px' }, speed);
  });
...
chatReload = function(){
	...
	logsControl();
	$.ajax({
		url: "system/action/action/chat_log.php",
		type: "post",
		cache: false,
		timeout: speed,
		dataType: 'json',
...
$(document).ready(function(){
	...
	var roomTitle =" اهلا وسهلا بك في شات حب وغزل";
...
$(document).ready(function() {
	...
	// استخدام الدالة لتحريك العنصر
	$('#example').animate({ left: '100px' }, speed);
  
	// استدعاء ملف PHP باستخدام طريقة GET
	$.get("system/action/system_load.php", function(data) {
	  console.log(data); // معالجة البيانات المستلمة
	});
  });
